...
RES1.DLNMpen.i.exp5<- vector("list", nsim)

for(i in 1:nsim) {
  
  print(i)
  
  RES1.DLNMpen.i.exp5[[i]] = applyDLNMpen(data.Y=resu.sim.data1.exp5[[i]],data.X=resu.sim.dataX.i[[i]]$X) #Fix before was resu.sim.dataX.i[[i]]
  RES1.DLNMpen.i.exp5[[i]]$numsim<-i
  RES1.DLNMpen.i.exp5[[i]]$true.pred<-ifelse(RES1.DLNMpen.i.exp5[[i]]$var %in% resu.sim.data1.exp5[[i]]$true.pred,1,0)
  # P-value of the crossbasis (overall p-value)
  RES1.DLNMpen.i.exp5[[i]]$DLNMpen.TP<-as.numeric(as.character(RES1.DLNMpen.i.exp5[[i]]$DLNMpen.TP))
  # Confidence interval of each lag includes 0 or not
  RES1.DLNMpen.i.exp5[[i]]$DLNMpen_lag.TP<-as.numeric(as.character(ifelse(RES1.DLNMpen.i.exp5[[i]]$DLNMpen_lag.TP=="TRUE",1,0)))
  # Store p-values (by step of 5 = overall p-value of the crossbasis)
  list.pval<-RES1.DLNMpen.i.exp5[[i]]$DLNMpen.TP[grep(".1",RES1.DLNMpen.i.exp5[[i]]$var,fixed = TRUE)]
  list.pval.BH<-rep(p.adjust(list.pval,"BH"),each=5)
  list.pval.BY<-rep(p.adjust(list.pval,"BY"),each=5)
  
  RES1.DLNMpen.i.exp5[[i]]$DLNMpen.none<-ifelse(RES1.DLNMpen.i.exp5[[i]]$DLNMpen_lag.TP %in% c(0),0,
                                                ifelse(RES1.DLNMpen.i.exp5[[i]]$DLNMpen.TP<0.05,1,0))
  RES1.DLNMpen.i.exp5[[i]]$DLNMpen.bonf<-ifelse(RES1.DLNMpen.i.exp5[[i]]$DLNMpen_lag.TP %in% c(0),0,
                                                ifelse(RES1.DLNMpen.i.exp5[[i]]$DLNMpen.TP<0.0005,1,0))
  RES1.DLNMpen.i.exp5[[i]]$DLNMpen.bh<-ifelse(RES1.DLNMpen.i.exp5[[i]]$DLNMpen_lag.TP %in% c(0),0,
                                              ifelse(list.pval.BH <0.05,1,0))
  RES1.DLNMpen.i.exp5[[i]]$DLNMpen.by<-ifelse(RES1.DLNMpen.i.exp5[[i]]$DLNMpen_lag.TP %in% c(0),0,
                                              ifelse(list.pval.BY <0.05,1,0))
  RES1.DLNMpen.i.exp5[[i]]<-RES1.DLNMpen.i.exp5[[i]][,c("var","numsim","true.pred","DLNMpen.TP","estimate","CI.inf","CI.sup","DLNMpen.none","DLNMpen.bonf","DLNMpen.bh","DLNMpen.by","DLNMpen_lag.TP")]
  # RES1.DLNMpen.i.exp5[[i]]$sign.lag<-ifelse(RES1.DLNMpen.i.exp5[[i]]$DLNMpen.TP>0.05,0,
  #                                      ifelse(RES1.DLNMpen.i.exp5[[i]]$DLNMpen.TP<0.05 & RES1.DLNMpen.i.exp5[[i]]$DLNMpen_lag.TP==0,0,1))
}

save(RES1.DLNMpen.i.exp5,file="dataY1andX/exp5/RES1.DLNMpen.i.exp5.RData")

#*#
load("dataY1andX/exp5/RES1.DLNMpen.i.exp5.RData")

for(i in 1:nsim) {
  RES1.DLNMpen.i.exp5[[i]]$DLNMpen.TP_dic <- ifelse(RES1.DLNMpen.i.exp5[[i]]$DLNMpen.TP<0.05,1,0)
}

save(RES1.DLNMpen.i.exp5,file="dataY1andX/exp5/RES1.DLNMpen.i.exp5.RData")
#*#

print("Step 4 - Completed model penalized DLNM without variable selection (output: RES1.DLNMpen.i.exp5.RData)")
...
RES1.DLNMselect.i.exp5 <- foreach(i = 1:nsim,.packages=c("foreach","Matrix","MASS","parallel","glmnet","spls","MXM","dlnm","splines","mgcv","doParallel",
  ...
  data.frame(applyDLNMselect(data.Y=resu.sim.data1.exp5[[i]],data.X=resu.sim.dataX.i[[i]]$X),i)
  
  }
    
save(RES1.DLNMselect.i.exp5,file="dataY1andX/exp5/RES1.DLNMselect.i.exp5.RData")

print("Step 5 - Completed model penalized DLNM with variable selection (output: RES1.DLNMselect.i.exp5.RData)")
...
save(RES1.DLNMselect.i.exp5.v2,file="dataY1andX/exp5/RES1.DLNMselect.i.exp5.v2.RData")

#*#
load("dataY1andX/exp5/RES1.DLNMselect.i.exp5.v2.RData")
...
print("Step 6 - Completed model penalized DLNM with variable selection with additional columns (output: RES1.DLNMselect.i.exp5.v2.RData)")
...
print("Step 7 - Completed model penalized DLNM with variable selection Backward prunning (output: RES1.DLNMselect.i.exp5.backward...)")
...
print("Step 8 - Completed model penalized DLNM with variable selection Backward prunning with additional columns (output: RES1.DLNMselect.i.exp5.backward.v2...)")

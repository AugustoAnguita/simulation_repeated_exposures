...
here::i_am("dataY2andX/exp5/data2exp5_sNPLS.R") # Declare the location of the current script and set wd / The project root is initialized (one step up), consistent with the location of the current script or report
...
save(RES2.sNPLS.i.exp5,file="RES2.sNPLS.i.exp5.RData")
...
# output_coefs$var[which(output_coefs$sNPLS.TP==1)] == resu.sim.data2.exp5[[1]]$true.pred
# 
# #Check results:
# sort(as.character(output_coefs$var[which(output_coefs$sNPLS.TP==1)])) == sort(as.character(resu.sim.data2.exp5[[1]]$true.pred))

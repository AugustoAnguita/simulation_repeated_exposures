...
RES2.DLNMpen.i.exp3<- vector("list", nsim)

for(i in 1:nsim) {
  
  print(i)
  
  RES2.DLNMpen.i.exp3[[i]] = applyDLNMpen(data.Y=resu.sim.data2.exp3[[i]],data.X=resu.sim.dataX.i[[i]]$X) #Fix before was resu.sim.dataX.i[[i]]
  RES2.DLNMpen.i.exp3[[i]]$numsim<-i
  RES2.DLNMpen.i.exp3[[i]]$true.pred<-ifelse(RES2.DLNMpen.i.exp3[[i]]$var %in% resu.sim.data2.exp3[[i]]$true.pred,1,0)
  # P-value of the crossbasis (overall p-value)
  RES2.DLNMpen.i.exp3[[i]]$DLNMpen.TP<-as.numeric(as.character(RES2.DLNMpen.i.exp3[[i]]$DLNMpen.TP))
  # Confidence interval of each lag includes 0 or not
  RES2.DLNMpen.i.exp3[[i]]$DLNMpen_lag.TP<-as.numeric(as.character(ifelse(RES2.DLNMpen.i.exp3[[i]]$DLNMpen_lag.TP=="TRUE",1,0)))
  # Store p-values (by step of 5 = overall p-value of the crossbasis)
  list.pval<-RES2.DLNMpen.i.exp3[[i]]$DLNMpen.TP[grep(".1",RES2.DLNMpen.i.exp3[[i]]$var,fixed = TRUE)]
  list.pval.BH<-rep(p.adjust(list.pval,"BH"),each=5)
  list.pval.BY<-rep(p.adjust(list.pval,"BY"),each=5)
  
  RES2.DLNMpen.i.exp3[[i]]$DLNMpen.none<-ifelse(RES2.DLNMpen.i.exp3[[i]]$DLNMpen_lag.TP %in% c(0),0,
                                                ifelse(RES2.DLNMpen.i.exp3[[i]]$DLNMpen.TP<0.05,1,0))
  RES2.DLNMpen.i.exp3[[i]]$DLNMpen.bonf<-ifelse(RES2.DLNMpen.i.exp3[[i]]$DLNMpen_lag.TP %in% c(0),0,
                                                ifelse(RES2.DLNMpen.i.exp3[[i]]$DLNMpen.TP<0.0005,1,0))
  RES2.DLNMpen.i.exp3[[i]]$DLNMpen.bh<-ifelse(RES2.DLNMpen.i.exp3[[i]]$DLNMpen_lag.TP %in% c(0),0,
                                              ifelse(list.pval.BH <0.05,1,0))
  RES2.DLNMpen.i.exp3[[i]]$DLNMpen.by<-ifelse(RES2.DLNMpen.i.exp3[[i]]$DLNMpen_lag.TP %in% c(0),0,
                                              ifelse(list.pval.BY <0.05,1,0))
  RES2.DLNMpen.i.exp3[[i]]<-RES2.DLNMpen.i.exp3[[i]][,c("var","numsim","true.pred","DLNMpen.TP","estimate","CI.inf","CI.sup","DLNMpen.none","DLNMpen.bonf","DLNMpen.bh","DLNMpen.by","DLNMpen_lag.TP")]
  # RES2.DLNMpen.i.exp3[[i]]$sign.lag<-ifelse(RES2.DLNMpen.i.exp3[[i]]$DLNMpen.TP>0.05,0,
  #                                      ifelse(RES2.DLNMpen.i.exp3[[i]]$DLNMpen.TP<0.05 & RES2.DLNMpen.i.exp3[[i]]$DLNMpen_lag.TP==0,0,1))
}

save(RES2.DLNMpen.i.exp3,file="dataY2andX/exp3/RES2.DLNMpen.i.exp3.RData")

#*#
load("dataY2andX/exp3/RES2.DLNMpen.i.exp3.RData")

for(i in 1:nsim) {
  RES2.DLNMpen.i.exp3[[i]]$DLNMpen.TP_dic <- ifelse(RES2.DLNMpen.i.exp3[[i]]$DLNMpen.TP<0.05,1,0)
}

save(RES2.DLNMpen.i.exp3,file="dataY2andX/exp3/RES2.DLNMpen.i.exp3.RData")
#*#

print("Step 4 - Completed model penalized DLNM without variable selection (output: RES2.DLNMpen.i.exp3.RData)")
...
RES2.DLNMselect.i.exp3 <- foreach(i = 1:nsim,.packages=c("foreach","Matrix","MASS","parallel","glmnet","spls","MXM","dlnm","splines","mgcv","doParallel",
  ...
  data.frame(applyDLNMselect(data.Y=resu.sim.data2.exp3[[i]],data.X=resu.sim.dataX.i[[i]]$X),i)
  
  }
    
save(RES2.DLNMselect.i.exp3,file="dataY2andX/exp3/RES2.DLNMselect.i.exp3.RData")

print("Step 5 - Completed model penalized DLNM with variable selection (output: RES2.DLNMselect.i.exp3.RData)")
...
save(RES2.DLNMselect.i.exp3.v2,file="dataY2andX/exp3/RES2.DLNMselect.i.exp3.v2.RData")


#*#
load("dataY2andX/exp3/RES2.DLNMselect.i.exp3.v2.RData")
...
print("Step 6 - Completed model penalized DLNM with variable selection with additional columns (output: RES2.DLNMselect.i.exp3.v2.RData)")
...
print("Step 7 - Completed model penalized DLNM with variable selection Backward prunning (output: RES2.DLNMselect.i.exp3.backward...)")
...
print("Step 8 - Completed model penalized DLNM with variable selection Backward prunning with additional columns (output: RES2.DLNMselect.i.exp3.backward.v2...)")
